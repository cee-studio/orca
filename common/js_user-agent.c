...
jsua_cleanup(js_State *J, void *p_ua)
{
  ...
  if (p_ua)
    ua_cleanup((struct user_agent *)p_ua);
}
...
jsua_ua_init(js_State *J)
{
  ...
  js_newuserdata(J, "UserAgent", NULL, NULL);
  {
    ...
    js_newcfunction(
      J, &UserAgent_prototype_string, "UserAgent.prototype.string", 1);
    js_defproperty(J, -2, "string", JS_DONTENUM);

    /* UserAgent.prototype.addHeader = function() { ... } */
    js_newcfunction(
      J, &UserAgent_prototype_addHeader, "UserAgent.prototype.addHeader", 2);
    js_defproperty(J, -2, "addHeader", JS_DONTENUM);

    /* UserAgent.prototype.setUrl = function() { ... } */
    js_newcfunction(
      J, &UserAgent_prototype_setUrl, "UserAgent.prototype.setUrl", 2);
