...
static void enable_debug_track(js_State *J)
{
  debug_stack = 1;
}

static void jsua_logger(js_State *J)
{
...
static void jsua_print(js_State *J)
{
  int i;
  int top = js_gettop(J);
  for (i = 1; i < top; ++i) {
...
static void jsua_cleanup(js_State *J, void *p_ua)
{
  jsua_log("jsua_cleanup is called\n");
  if (p_ua) ua_cleanup((struct user_agent *)p_ua);
}

static void new_UserAgent(js_State *J)
{
...
static void UserAgent_prototype_run(js_State *J)
{
...
static void UserAgent_prototype_string(js_State *J)
{
  struct user_agent *ua = js_touserdata(J, 0, "UserAgent");
  struct ua_info info = { 0 };
  jsua_run(J, ua, &info);
...
static void UserAgent_prototype_addHeader(js_State *J)
{
  if (!js_isstring(J, 1))
...
static void UserAgent_prototype_setUrl(js_State *J)
{
  if (!js_isstring(J, 1)) {
    js_typeerror(J, "Expected 'first' argument to be a 'string'");
  }
...
static void jsua_ua_init(js_State *J)
{
  js_getglobal(J, "Object");
...
ORCAcode jsua_run(js_State *J, struct user_agent *ua, struct ua_info *p_info)
{
  if (!js_isstring(J, 1))
    js_typeerror(J, "Expected 'first' argument to be a 'string'");
  if (!js_isstring(J, 2))
