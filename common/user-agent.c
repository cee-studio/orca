...
struct user_agent {
   ...
   */
  struct {
    uint64_t blockuntil_tstamp; /**< lock every active conn from conn_pool
                                   until timestamp */
...
http_method_eval(char method[])
{
  ...
  if (STREQ(method, "POST"))
    return HTTP_POST;
  if (STREQ(method, "MIMEPOST"))
    return HTTP_MIMEPOST;
...
ua_reqheader_del(struct user_agent *ua, const char field[])
{
  ...
  logconf_warn(
    &ua->conf, "Couldn't find field '%s' in existing request header", field);
}
...
conn_respheader_cb(char *buf, size_t size, size_t nmemb, void *p_userdata)
{
  ...
  char *ptr;
  if (!(ptr = strchr(
          buf, ':'))) { /* returns if can't find ':' field/value delimiter */
    return bufsize;
  }
...
void
ua_curl_easy_setopt(struct user_agent *ua, void *data,
                    void(setopt_cb)(CURL *ehandle, void *data))
{
...
void
ua_curl_mime_setopt(struct user_agent *ua, void *data,
...
get_conn(struct user_agent *ua)
{
  ...
  VASSERT_S(NULL != ret_conn,
            "[%s] (Internal error) Couldn't fetch conn",
            ua->conf.id);
...
ua_init(struct logconf *conf)
{
  ...
  /* default header */
  ua_reqheader_add(
    new_ua, "User-Agent", "Orca (https://github.com/cee-studio/orca)");
...
ua_clone(struct user_agent *orig_ua)
{
  ...
  clone_ua->base_url.size = asprintf(&clone_ua->base_url.start,
                                     "%.*s",
                                     (int)orig_ua->base_url.size,
...
set_url(struct user_agent *ua, struct _ua_conn *conn, char endpoint[])
{
  ...
  size_t ret = snprintf(conn->info.req_url.start,
                        conn->info.req_url.size,
                        "%.*s",
                        (int)ua->base_url.size,
                        ua->base_url.start);
  ASSERT_S(ret < conn->info.req_url.size, "Out of bounds write attempt");
  ret += snprintf(conn->info.req_url.start + ret,
                  conn->info.req_url.size - ret,
                  "%s",
                  endpoint);
  ASSERT_S(ret < conn->info.req_url.size, "Out of bounds write attempt");
                ...
                struct ua_resp_handle *resp_handle)
{
  ...
  if (conn->info.httpcode >= 500 && conn->info.httpcode < 600) {
    ...
    if (resp_handle) {
      if (resp_handle->err_cb) {
        (*resp_handle->err_cb)(
          conn->info.body.buf, conn->info.body.len, resp_handle->err_obj);
      }
      else if (resp_handle->cxt_err_cb) {
        (*resp_handle->cxt_err_cb)(resp_handle->cxt,
                                   conn->info.body.buf,
                                   conn->info.body.len,
                                   resp_handle->err_obj);
      }
  ...
  if (conn->info.httpcode >= 200) {
    ...
    if (resp_handle) {
      if (resp_handle->ok_cb) {
        (*resp_handle->ok_cb)(
          conn->info.body.buf, conn->info.body.len, resp_handle->ok_obj);
      }
      else if (resp_handle->cxt_ok_cb) {
        (*resp_handle->cxt_ok_cb)(resp_handle->cxt,
                                  conn->info.body.buf,
                                  conn->info.body.len,
                                  resp_handle->ok_obj);
      }
  ...
  logconf_error(
    conn->conf, "Unusual HTTP response code: %d", conn->info.httpcode);
       ...
       enum http_method http_method, char endpoint[])
{
  ...
  if (info) {
    ...
    asprintf(
      &info->body.buf, "%.*s", (int)conn->info.body.len, conn->info.body.buf);
    asprintf(&info->header.buf,
             "%.*s",
             (int)conn->info.header.len,
             conn->info.header.buf);
    asprintf(&info->req_url.start,
             "%.*s",
             (int)conn->info.req_url.size,
             conn->info.req_url.start);
  }
