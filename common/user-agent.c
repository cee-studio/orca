...
#define CURLE_CHECK(conn, ecode)                                               \
  VASSERT_S(CURLE_OK == ecode, "[%s] (CURLE code: %d) %s", conn->conf->id,     \
            ecode,                                                             \
...
void ua_reqheader_add(struct user_agent* ua, const char field[],
                      const char value[])
{
  char buf[4096];
...
void ua_reqheader_del(struct user_agent* ua, const char field[])
{
  ...
  logconf_warn(&ua->conf, "Couldn't find field '%s' in existing request header",
               field);
}
 ...
 */
static size_t conn_respheader_cb(char* buf, size_t size, size_t nmemb,
                                 void* p_userdata)
{
 ...
 */
static size_t conn_respbody_cb(char* buf, size_t size, size_t nmemb,
                               void* p_userdata)
{
...
void ua_curl_easy_setopt(struct user_agent* ua, void* data,
                         void(setopt_cb)(CURL* ehandle, void* data))
{
...
void ua_curl_mime_setopt(struct user_agent* ua, void* data,
                         curl_mime*(mime_cb)(CURL* ehandle, void* data))
{
  ua->mime_cb = mime_cb;
  ua->data2 = data;
}

static struct _ua_conn* conn_init(struct user_agent* ua)
{
  ...
  ecode = curl_easy_setopt(new_ehandle, CURLOPT_WRITEDATA,
                           &new_conn->info.body);
  CURLE_CHECK(new_conn, ecode);
  ...
  ecode = curl_easy_setopt(new_ehandle, CURLOPT_HEADERDATA,
...
static void conn_cleanup(struct _ua_conn* conn)
{
...
static void conn_reset(struct _ua_conn* conn)
{
  conn->is_busy = false;
...
static struct _ua_conn* get_conn(struct user_agent* ua)
{
...
struct user_agent* ua_clone(struct user_agent* orig_ua)
{
  ...
  clone_ua->base_url.size =
          asprintf(&clone_ua->base_url.start, "%.*s",
...
static void set_method(struct user_agent* ua, struct _ua_conn* conn,
...
static void set_url(struct user_agent* ua, struct _ua_conn* conn,
                    char endpoint[])
{
  ...
  if (url_len > conn->info.req_url.size) {
    ...
    conn->info.req_url = (struct sized_buffer){ .start = tmp, .size = url_len };
  }
  ...
  ret += snprintf(conn->info.req_url.start + ret, conn->info.req_url.size - ret,
                  "%s", endpoint);
  ASSERT_S(ret < conn->info.req_url.size, "Out of bounds write attempt");

  CURLcode ecode = curl_easy_setopt(conn->ehandle, CURLOPT_URL,
...
static int send_request(struct user_agent* ua, struct _ua_conn* conn)
{
  pthread_mutex_lock(&ua->shared->lock);
  ...
  if (CURLE_READ_ERROR == ecode &&
    ...
    logconf_warn(conn->conf,
                 "The remote server closes connection without terminating SSL");
  else
    CURLE_CHECK(conn, ecode);
#else
  CURLE_CHECK(conn, ecode);
...
static ORCAcode perform_request(struct user_agent* ua, struct _ua_conn* conn,
