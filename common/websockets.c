...
#define CURLE_CHECK(ws, ecode)                                                 \
  VASSERT_S(CURLE_OK == ecode, "[%s] (CURLE code: %d) %s", ws->conf.id, ecode, \
            IS_EMPTY_STRING(ws->errbuf) ? curl_easy_strerror(ecode) :          \
                                          ws->errbuf)

#define CURLM_CHECK(ws, mcode)                                                 \
  VASSERT_S(CURLM_OK == mcode, "[%s] (CURLM code: %d) %s", ws->conf.id, mcode, \
...
static int _curl_debug_trace(CURL *handle, curl_infotype type, char *data,
...
static int _curl_tls_check(CURL *handle, curl_infotype type, char *data,
                           size_t size, void *userp)
{
  (void)handle;
  (void)data;
...
static const char *_ws_status_print(enum ws_status status)
{
  switch (status) {
...
static void _ws_set_status_nolock(struct websockets *ws, enum ws_status status)
{
...
static void cws_on_connect_cb(void *p_ws, CURL *ehandle,
                              const char *ws_protocols)
{
  ...
  logconf_http(&ws->conf, &ws->info.loginfo, ws->base_url,
               ...
               (struct sized_buffer){ (char *)ws_protocols,
                                      strlen(ws_protocols) },
               "WS_RCV_CONNECT");
...
static void cws_on_close_cb(void *p_ws, CURL *ehandle,
                            enum cws_close_reason cwscode, const char *reason,
                            size_t len)
{
  struct websockets *ws = p_ws;
  ...
  logconf_http(&ws->conf, &ws->info.loginfo, ws->base_url,
               (struct sized_buffer){ "", 0 },
               (struct sized_buffer){ (char *)reason, len }, "WS_RCV_CLOSE(%d)",
               cwscode);
...
static void cws_on_text_cb(void *p_ws, CURL *ehandle, const char *text,
                           size_t len)
{
  struct websockets *ws = p_ws;
  ...
  logconf_trace(&ws->conf,
                ANSICOLOR("RCV",
                          ANSI_FG_YELLOW) " TEXT (%zu bytes) [@@@_%zu_@@@]",
                len, ws->info.loginfo.counter);

  (*ws->cbs.on_text)(ws->cbs.data, ws, &ws->info, text, len);
}

static void cws_on_binary_cb(void *p_ws, CURL *ehandle, const void *mem,
                             size_t len)
{
  struct websockets *ws = p_ws;
  ...
  logconf_trace(&ws->conf,
                ANSICOLOR("RCV",
                          ANSI_FG_YELLOW) " BINARY (%zu bytes) [@@@_%zu_@@@]",
                len, ws->info.loginfo.counter);

  (*ws->cbs.on_binary)(ws->cbs.data, ws, &ws->info, mem, len);
}

static void cws_on_ping_cb(void *p_ws, CURL *ehandle, const char *reason,
                           size_t len)
{
  struct websockets *ws = p_ws;
  ...
  logconf_trace(&ws->conf,
                ANSICOLOR("RCV",
                          ANSI_FG_YELLOW) " PING (%zu bytes) [@@@_%zu_@@@]",
                len, ws->info.loginfo.counter);

  (*ws->cbs.on_ping)(ws->cbs.data, ws, &ws->info, reason, len);
}

static void cws_on_pong_cb(void *p_ws, CURL *ehandle, const char *reason,
                           size_t len)
{
  struct websockets *ws = p_ws;
  ...
  logconf_trace(&ws->conf,
                ANSICOLOR("RCV",
                          ANSI_FG_YELLOW) " PONG (%zu bytes) [@@@_%zu_@@@]",
                len, ws->info.loginfo.counter);

  (*ws->cbs.on_pong)(ws->cbs.data, ws, &ws->info, reason, len);
}

static bool _ws_close(struct websockets *ws, enum ws_close_reason code,
                      const char reason[]);

static int _ws_check_action_cb(void *p_userdata, curl_off_t dltotal,
...
static CURL *_ws_cws_new(struct websockets *ws, const char ws_protocols[])
{
  ...
  ecode = curl_easy_setopt(new_ehandle, CURLOPT_DEBUGFUNCTION,
                           _curl_debug_trace);
  CURLE_CHECK(ws, ecode);
                      ...
                      const char reason[])
{
  ...
  if (WS_DISCONNECTED == ws->status) {
    logconf_warn(
            ...
            ANSICOLOR(
                    "Failed",
                    ANSI_FG_RED) " at SEND CLOSE : Connection already closed [@@@_%zu_@@@]",
  ...
  if (WS_DISCONNECTING == ws->status) {
    logconf_warn(
            ...
            ANSICOLOR(
                    "Failed",
                    ANSI_FG_RED) " at SEND CLOSE : Close already taking place [@@@_%zu_@@@]",
            ws->info.loginfo.counter);
    return false;
  }
...
static void noop_on_connect(void *a, struct websockets *b, struct ws_info *info,
                            const char *c)
{
  return;
}
static void noop_on_text(void *a, struct websockets *b, struct ws_info *info,
                         const char *c, size_t d)
{
  return;
}
static void noop_on_binary(void *a, struct websockets *b, struct ws_info *info,
                           const void *c, size_t d)
{
  return;
}
static void noop_on_ping(void *a, struct websockets *ws, struct ws_info *info,
                         const char *reason, size_t len)
{
  ws_pong(ws, &ws->info, reason, len);
}
static void noop_on_pong(void *a, struct websockets *b, struct ws_info *info,
                         const char *c, size_t d)
{
  return;
}
static void noop_on_close(void *a, struct websockets *b, struct ws_info *info,
                          enum ws_close_reason c, const char *d, size_t e)
{
  return;
}

struct websockets *ws_init(struct ws_callbacks *cbs, struct logconf *conf)
{
...
bool ws_send_binary(struct websockets *ws, struct ws_info *info,
                    const char msg[], size_t msglen)
{
  ...
  logconf_trace(&ws->conf,
                ANSICOLOR("SEND",
                          ANSI_FG_GREEN) " BINARY (%zu bytes) [@@@_%zu_@@@]",
  ...
  if (!cws_send(ws->ehandle, false, msg, msglen)) {
    logconf_error(&ws->conf,
                  ANSICOLOR("Failed",
                            ANSI_FG_RED) " at SEND BINARY [@@@_%zu_@@@]",
                  ws->info.loginfo.counter);
    return false;
  }
  return true;
...
bool ws_send_text(struct websockets *ws, struct ws_info *info,
                  const char text[], size_t len)
{
  ...
  logconf_trace(&ws->conf,
                ANSICOLOR("SEND",
                          ANSI_FG_GREEN) " TEXT (%zu bytes) [@@@_%zu_@@@]",
                len, ws->info.loginfo.counter);
  ...
  if (!cws_send(ws->ehandle, true, text, len)) {
    logconf_error(&ws->conf,
                  ANSICOLOR("Failed",
                            ANSI_FG_RED) " at SEND TEXT [@@@_%zu_@@@]",
                  ws->info.loginfo.counter);
    return false;
  }
  return true;
...
bool ws_ping(struct websockets *ws, struct ws_info *info, const char *reason,
             size_t len)
{
/* disabled because this creates too many entries */
  ...
  logconf_trace(&ws->conf,
                ANSICOLOR("SEND",
                          ANSI_FG_GREEN) " PING (%zu bytes) [@@@_%zu_@@@]",
                len, ws->info.loginfo.counter);
...
bool ws_pong(struct websockets *ws, struct ws_info *info, const char *reason,
             size_t len)
{
/* disabled because this creates too many entries */
#if 0
  ...
  logconf_trace(&ws->conf,
                ANSICOLOR("SEND",
                          ANSI_FG_GREEN) " PONG (%zu bytes) [@@@_%zu_@@@]",
                len, ws->info.loginfo.counter);
...
void ws_start(struct websockets *ws)
{
  ...
  VASSERT_S(
          false == ws_is_alive(ws),
          "[%s] Please shutdown current WebSockets connection before calling ws_start() (Current status: %s)",
          ws->conf.id, _ws_status_print(ws->status));
  VASSERT_S(
          NULL == ws->ehandle,
          "[%s] (Internal error) Attempt to reconnect without properly closing the connection",
          ws->conf.id);
...
void ws_perform(struct websockets *ws, bool *p_is_running, uint64_t wait_ms)
{
  ASSERT_S(ws->tid == pthread_self(),
...
void ws_close(struct websockets *ws, const enum ws_close_reason code,
              const char reason[], const size_t len)
{
