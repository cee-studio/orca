...
static int
_curl_debug_trace(CURL *handle, curl_infotype type, char *data, size_t size,
                  void *userp)
{
  (void)handle;
  (void)userp;
...
static int
_curl_tls_check(CURL *handle, curl_infotype type, char *data, size_t size,
                void *userp)
{
  (void)handle;
  (void)data;
...
static void
cws_on_close_cb(void *p_ws, CURL *ehandle, enum cws_close_reason cwscode,
...
static int
_ws_check_action_cb(void *p_userdata, curl_off_t dltotal, curl_off_t dlnow,
...
_ws_cws_new(struct websockets *ws, const char ws_protocols[])
{
  ...
  ecode = curl_easy_setopt(
    new_ehandle, CURLOPT_XFERINFOFUNCTION, &_ws_check_action_cb);
  CURLE_CHECK(ws, ecode);
...
static void
noop_on_connect(void *a, struct websockets *b, struct ws_info *info,
                const char *c)
{
  return;
}
static void
noop_on_text(void *a, struct websockets *b, struct ws_info *info,
             const char *c, size_t d)
{
  return;
}
static void
noop_on_binary(void *a, struct websockets *b, struct ws_info *info,
               const void *c, size_t d)
{
  return;
}
static void
noop_on_ping(void *a, struct websockets *ws, struct ws_info *info,
             const char *reason, size_t len)
{
  ws_pong(ws, &ws->info, reason, len);
}
static void
noop_on_pong(void *a, struct websockets *b, struct ws_info *info,
             const char *c, size_t d)
{
  return;
}
static void
noop_on_close(void *a, struct websockets *b, struct ws_info *info,
           ...
           const char ws_protocols[])
{
  ...
  VASSERT_S(ret < sizeof(ws->base_url),
            "[%s] Out of bounds write attempt",
            ws->conf.id);

  if (!IS_EMPTY_STRING(ws_protocols)) {
    ret = snprintf(ws->protocols, sizeof(ws->protocols), "%s", ws_protocols);
    VASSERT_S(ret < sizeof(ws->protocols),
              "[%s] Out of bounds write attempt",
              ws->conf.id);
  }
...
bool
ws_send_binary(struct websockets *ws, struct ws_info *info, const char msg[],
               size_t msglen)
{
...
bool
ws_send_text(struct websockets *ws, struct ws_info *info, const char text[],
             size_t len)
{
  ASSERT_S(ws->tid == pthread_self(), "Can only be called from main-thread");
...
bool
ws_ping(struct websockets *ws, struct ws_info *info, const char *reason,
        size_t len)
{
...
bool
ws_pong(struct websockets *ws, struct ws_info *info, const char *reason,
        size_t len)
{
/* disabled because this creates too many entries */
#if 0
...
void
ws_close(struct websockets *ws, const enum ws_close_reason code,
         const char reason[], const size_t len)
{
  logconf_warn(&ws->conf,
               "Attempting to close WebSockets connection with %s : %.*s",
               ws_close_opcode_print(code),
               (int)len,
               reason);
  ...
  snprintf(ws->pending_close.reason,
           sizeof(ws->pending_close.reason),
           "%.*s",
           (int)len,
           reason);
  pthread_mutex_unlock(&ws->lock);
