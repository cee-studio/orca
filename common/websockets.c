...
cws_on_connect_cb(void *p_ws, CURL *ehandle, const char *ws_protocols)
{
  ...
  logconf_trace(
    &ws->conf,
    ANSICOLOR(
      "RCV", ANSI_FG_YELLOW) " CONNECT (WS-Protocols: '%s') [@@@_%zu_@@@]",
    ws_protocols,
...
static bool _ws_close(
  struct websockets *ws, enum ws_close_reason code, const char reason[]);
...
static void
noop_on_connect(
  void *a, struct websockets *b, struct ws_info *info, const char *c)
{
  return;
}
static void
...
void
ws_set_url(
  struct websockets *ws, const char base_url[], const char ws_protocols[])
{
  pthread_mutex_lock(&ws->lock);
...
bool
ws_send_binary(
  struct websockets *ws, struct ws_info *info, const char msg[], size_t msglen)
{
...
bool
ws_send_text(
  struct websockets *ws, struct ws_info *info, const char text[], size_t len)
{
  ASSERT_S(ws->tid == pthread_self(), "Can only be called from main-thread");
...
bool
ws_ping(
  struct websockets *ws, struct ws_info *info, const char *reason, size_t len)
{
/* disabled because this creates too many entries */
  ...
  if (!cws_ping(ws->ehandle, reason, len)) {
    logconf_error(
      &ws->conf, ANSICOLOR("Failed", ANSI_FG_RED) " at SEND PING.");
    return false;
  }
  return true;
}
...
/* disabled because this creates too many entries */
  ...
  if (!cws_pong(ws->ehandle, reason, len)) {
    logconf_error(
      &ws->conf, ANSICOLOR("Failed", ANSI_FG_RED) " at SEND PONG.");
    return false;
  }
  return true;
}
...
ws_start(struct websockets *ws)
{
  ...
  VASSERT_S(
    ...
    "[%s] Please shutdown current WebSockets connection before calling "
    "ws_start() (Current status: %s)",
    ws->conf.id,
    _ws_status_print(ws->status));
  VASSERT_S(
    NULL == ws->ehandle,
    "[%s] (Internal error) Attempt to reconnect without properly closing the "
    "connection",
    ws->conf.id);
