                          ...
                          struct sized_buffer *token)
{
  ...
  if (!token->size) { /* no token means a webhook-only client */
    ...
    char auth[128];
    int ret = snprintf(auth, sizeof(auth), "Bot %.*s", (int)token->size,
                       token->start);
 ...
 */
static void json_error_cb(char *str, size_t len, void *p_adapter)
{
  ...
  snprintf(adapter->err.jsonstr, sizeof(adapter->err.jsonstr), "%.*s", (int)len,
           str);
}

static ORCAcode _discord_perform_request(
        struct discord_adapter *adapter, struct ua_resp_handle *resp_handle,
        struct sized_buffer *req_body, enum http_method http_method,
        char endpoint[], struct discord_bucket *bucket, const char route[])
{
  bool keepalive = true;
  long delay_ms;
  ORCAcode code;
  ...
  do {
    ...
    if (code != ORCA_HTTP_CODE) {
      ...
      case HTTP_METHOD_NOT_ALLOWED:
        ...
        logconf_fatal(
                &adapter->conf,
                "METHOD_NOT_ALLOWED: The server couldn't recognize the received HTTP method");
        break;
      case HTTP_TOO_MANY_REQUESTS: {
        ...
        if (is_global) {
          ...
          logconf_warn(&adapter->conf, "429 RATELIMITING (wait: %.2lf ms) : %s",
                       retry_after, message);
