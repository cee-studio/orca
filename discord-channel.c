...
ORCAcode
discord_get_channel(struct discord *client, const u64_snowflake_t channel_id,
                        ...
                        const char emoji_name[])
{
  ...
  if (emoji_id)
    snprintf(emoji_endpoint,
             sizeof(emoji_endpoint),
             "%s:%" PRIu64,
             pct_emoji_name,
             emoji_id);
  else
...
ORCAcode
discord_get_reactions(struct discord *client, u64_snowflake_t channel_id,
                      u64_snowflake_t message_id,
                      const u64_snowflake_t emoji_id, const char emoji_name[],
...
ORCAcode
discord_edit_message(struct discord *client, const u64_snowflake_t channel_id,
                     const u64_snowflake_t message_id,
...
ORCAcode
discord_delete_message(struct discord *client, u64_snowflake_t channel_id,
                       u64_snowflake_t message_id)
{
  if (!channel_id) {
    log_error("Missing 'channel_id'");
                             ...
                             NTL_T(u64_snowflake_t) messages)
{
  ...
  size_t ret = json_ainject(
    &payload, "(messages):F", ja_u64_list_to_json, (NTL_T(ja_u64))messages);
...
ORCAcode
discord_pin_message(struct discord *client, const u64_snowflake_t channel_id,
                    const u64_snowflake_t message_id)
{
  if (!channel_id) {
    log_error("Missing 'channel_id'");
...
ORCAcode
discord_unpin_message(struct discord *client, const u64_snowflake_t channel_id,
                      const u64_snowflake_t message_id)
{
  if (!channel_id) {
    log_error("Missing 'channel_id'");
                                     ...
                                     struct discord_thread_response_body *body)
{
  ...
  if (before) {
    offset += snprintf(
      query + offset, sizeof(query) - offset, "before=%" PRIu64, before);
    ASSERT_S(offset < sizeof(query), "Out of bounds write attempt");
  }
  if (limit) {
    offset += snprintf(query + offset,
