...
_discord_init(struct discord *new_client)
{
  discord_adapter_init(
    &new_client->adapter, new_client->conf, &new_client->token);
...
discord_config_init(const char config_file[])
{
  ...
  VASSERT_S(
    fp != NULL, "Couldn't open '%s': %s", config_file, strerror(errno));
  ...
  if (STRNEQ(
        "YOUR-BOT-TOKEN", new_client->token.start, new_client->token.size)) {
  ...
  struct discord *client, char *command, discord_message_cb callback)
{
  ...
  client->gw.user_cmd->pool = realloc(
  ...
  discord_add_intents(
    client, DISCORD_GATEWAY_GUILD_MESSAGES | DISCORD_GATEWAY_DIRECT_MESSAGES);
}
...
void
discord_set_on_sb_message_create(
...
void
discord_set_on_sb_message_update(
  struct discord *client, discord_sb_message_cb callback)
{
  ...
  struct discord *client, struct discord_presence_status *presence)
{
  ...
  memcpy(
    client->gw.id.presence, presence, sizeof(struct discord_presence_status));
}
