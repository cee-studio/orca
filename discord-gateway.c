...
static void sized_buffer_from_json(char* json, size_t len, void* data)
{
...
{
  char payload[64];
  int ret = json_inject(payload, sizeof(payload), "(op):1,(d):d",
                        &gw->payload->seq);
  ASSERT_S(ret < sizeof(payload), "Out of bounds write attempt");
...
static enum discord_gateway_events get_dispatch_event(char event_name[])
{
...
#define __RETURN_IF_MATCH(event, str)                                          \
...
static void on_guild_role_create(struct discord_gateway* gw,
...
static void on_guild_role_update(struct discord_gateway* gw,
                                 struct sized_buffer* data)
{
  struct discord_role* role = NULL;
...
static void on_guild_role_delete(struct discord_gateway* gw,
                                 struct sized_buffer* data)
{
...
static void on_guild_member_add(struct discord_gateway* gw,
                                struct sized_buffer* data)
{
...
static void on_guild_member_update(struct discord_gateway* gw,
                                   struct sized_buffer* data)
{
  struct discord_guild_member* member = NULL;
  discord_guild_member_from_json(data->start, data->size, &member);
...
static void on_guild_member_remove(struct discord_gateway* gw,
                                   struct sized_buffer* data)
{
  u64_snowflake_t guild_id = 0;
...
static void on_guild_ban_add(struct discord_gateway* gw,
                             struct sized_buffer* data)
{
  u64_snowflake_t guild_id = 0;
  struct discord_user* user = NULL;
...
static void on_guild_ban_remove(struct discord_gateway* gw,
                                struct sized_buffer* data)
{
  u64_snowflake_t guild_id = 0;
  struct discord_user* user = NULL;
...
static void on_application_command_create(struct discord_gateway* gw,
                                          struct sized_buffer* data)
{
...
static void on_application_command_update(struct discord_gateway* gw,
                                          struct sized_buffer* data)
{
  struct discord_application_command* cmd = NULL;
  discord_application_command_from_json(data->start, data->size, &cmd);
...
static void on_application_command_delete(struct discord_gateway* gw,
                                          struct sized_buffer* data)
{
  struct discord_application_command* cmd = NULL;
  discord_application_command_from_json(data->start, data->size, &cmd);
...
static void on_channel_create(struct discord_gateway* gw,
                              struct sized_buffer* data)
{
...
static void on_channel_update(struct discord_gateway* gw,
                              struct sized_buffer* data)
{
  struct discord_channel* channel = NULL;
  discord_channel_from_json(data->start, data->size, &channel);
...
static void on_channel_delete(struct discord_gateway* gw,
                              struct sized_buffer* data)
{
  struct discord_channel* channel = NULL;
  discord_channel_from_json(data->start, data->size, &channel);
...
static void on_channel_pins_update(struct discord_gateway* gw,
                                   struct sized_buffer* data)
{
...
static void on_thread_create(struct discord_gateway* gw,
                             struct sized_buffer* data)
{
...
static void on_thread_update(struct discord_gateway* gw,
                             struct sized_buffer* data)
{
  struct discord_channel* thread = NULL;
  discord_channel_from_json(data->start, data->size, &thread);
...
static void on_thread_delete(struct discord_gateway* gw,
                             struct sized_buffer* data)
{
  struct discord_channel* thread = NULL;
  discord_channel_from_json(data->start, data->size, &thread);
...
static void on_interaction_create(struct discord_gateway* gw,
                                  struct sized_buffer* data)
{
...
static void on_message_create(struct discord_gateway* gw,
                              struct sized_buffer* data)
{
  ...
  if (gw->user_cmd->cbs.sb_on_message_create) /* @todo temporary */
    (*gw->user_cmd->cbs.sb_on_message_create)(CLIENT(gw), &gw->bot, &gw->sb_bot,
                                              msg, data);
...
static void on_message_update(struct discord_gateway* gw,
                              struct sized_buffer* data)
{
  struct discord_message* msg = NULL;
  discord_message_from_json(data->start, data->size, &msg);
  ...
  if (gw->user_cmd->cbs.sb_on_message_update)
    (*gw->user_cmd->cbs.sb_on_message_update)(CLIENT(gw), &gw->bot, &gw->sb_bot,
                                              msg, data);
...
static void on_message_delete(struct discord_gateway* gw,
                              struct sized_buffer* data)
{
...
static void on_message_delete_bulk(struct discord_gateway* gw,
                                   struct sized_buffer* data)
{
...
static void on_message_reaction_add(struct discord_gateway* gw,
                                    struct sized_buffer* data)
{
...
static void on_message_reaction_remove(struct discord_gateway* gw,
                                       struct sized_buffer* data)
{
  u64_snowflake_t user_id = 0, message_id = 0, channel_id = 0, guild_id = 0;
  struct discord_emoji* emoji = NULL;
...
static void on_message_reaction_remove_all(struct discord_gateway* gw,
                                           struct sized_buffer* data)
{
...
static void on_message_reaction_remove_emoji(struct discord_gateway* gw,
                                             struct sized_buffer* data)
{
...
static void on_voice_state_update(struct discord_gateway* gw,
                                  struct sized_buffer* data)
{
...
static void on_voice_server_update(struct discord_gateway* gw,
                                   struct sized_buffer* data)
{
  u64_snowflake_t guild_id = 0;
...
static void on_dispatch(struct discord_gateway* gw)
{
  ...
  mode = gw->user_cmd->scheduler(CLIENT(gw), &gw->bot, &gw->payload->event_data,
                                 event);
  switch (mode) {
...
static void on_connect_cb(void* p_gw, struct websockets* ws,
                          struct ws_info* info, const char* ws_protocols)
{
  struct discord_gateway* gw = p_gw;
  logconf_info(&gw->conf, "Connected, WS-Protocols: '%s'", ws_protocols);
}

static void on_close_cb(void* p_gw, struct websockets* ws, struct ws_info* info,
                        enum ws_close_reason wscode, const char* reason,
                        size_t len)
{
  struct discord_gateway* gw = p_gw;
  ...
  logconf_warn(&gw->conf,
               ANSICOLOR("CLOSE %s",
                         ANSI_FG_RED) " (code: %4d, %zu bytes): '%.*s'",
...
static void on_text_cb(void* p_gw, struct websockets* ws, struct ws_info* info,
                       const char* text, size_t len)
{
  ...
  logconf_trace(
          ...
          (*gw->payload->event_name) ? " -> " : "", gw->payload->event_name,
          len, info->loginfo.counter);
...
static void noop_event_raw_cb(struct discord* a, enum discord_gateway_events b,
                              struct sized_buffer* c, struct sized_buffer* d)
{
  return;
}
static enum discord_event_scheduler
noop_scheduler(struct discord* a, struct discord_user* b,
               struct sized_buffer* c, enum discord_gateway_events d)
{
  return DISCORD_EVENT_MAIN_THREAD;
}

void discord_gateway_init(struct discord_gateway* gw, struct logconf* conf,
                          struct sized_buffer* token)
{
  ...
  *gw->id.properties = (struct discord_identify_connection){ .os = "POSIX",
                                                             .browser = "orca",
                                                             .device = "orca" };
  ...
  struct sized_buffer default_prefix =
          logconf_get_field(conf, "discord.default_prefix");
  if (default_prefix.size) {
    ...
    if (enable_prefix && prefix) {
      gw->user_cmd->prefix =
              (struct sized_buffer){ .start = prefix, .size = strlen(prefix) };
    }
  }
}
 ...
 */
static ORCAcode event_loop(struct discord_gateway* gw)
{
