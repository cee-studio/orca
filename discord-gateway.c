...
send_identify(struct discord_gateway *gw)
{
  ...
  if ((ws_timestamp(gw->ws) - gw->session.identify_tstamp) < 5) {
    ...
    VASSERT_S(
      gw->session.concurrent < gw->session.start_limit.max_concurrency,
  ...
  logconf_info(
    &gw->conf,
    ANSICOLOR(
      "SEND", ANSI_FG_BRIGHT_GREEN) " IDENTIFY (%d bytes) [@@@_%zu_@@@]",
    ret,
    info.loginfo.counter + 1);
...
send_heartbeat(struct discord_gateway *gw)
{
  ...
  logconf_info(
    &gw->conf,
    ANSICOLOR(
      "SEND", ANSI_FG_BRIGHT_GREEN) " HEARTBEAT (%d bytes) [@@@_%zu_@@@]",
    ret,
    info.loginfo.counter + 1);
...
on_application_command_create(
  struct discord_gateway *gw, struct sized_buffer *data)
{
...
on_message_create(struct discord_gateway *gw, struct sized_buffer *data)
{
  ...
  if (
    gw->user_cmd->pool &&
    STRNEQ(
      gw->user_cmd->prefix.start, msg->content, gw->user_cmd->prefix.size)) {
...
on_dispatch(struct discord_gateway *gw)
{
  ...
  case DISCORD_GATEWAY_EVENTS_READY:
    ...
    json_extract(
      ...
      gw->session_id);
    ASSERT_S(
      !IS_EMPTY_STRING(gw->session_id), "Missing session_id from READY event");
  ...
  case DISCORD_GATEWAY_EVENTS_MESSAGE_CREATE:
    if (
      gw->user_cmd->pool || gw->user_cmd->cbs.sb_on_message_create ||
  ...
  case DISCORD_EVENT_WORKER_THREAD: {
    ...
    *p_cxt = (struct discord_event_cxt){
      ...
      .p_gw = &client_cpy->gw,
      .data = { .start = strndup(
                  gw->payload->event_data.start, gw->payload->event_data.size),
  ...
  size_t len)
{
  ...
  logconf_trace(
    &gw->conf,
    ANSICOLOR(
      "RCV", ANSI_FG_BRIGHT_YELLOW) " %s%s%s (%zu bytes) [@@@_%zu_@@@]",
...
discord_gateway_init(
  struct discord_gateway *gw, struct logconf *conf, struct sized_buffer *token)
{
