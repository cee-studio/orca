...
on_dispatch(struct discord_gateway *gw)
{
  ...
  switch (event = get_dispatch_event(gw->payload->event_name)) {
                     ...
                     struct sized_buffer *token)
{
  ...
  if (buf.size) {
    ...
    if (enable_prefix) {
      ...
      gw->user_cmd->prefix.start = prefix;
      gw->user_cmd->prefix.size = prefix ? strlen(prefix) : 0;
    }
  }
}
