...
send_resume(struct discord_gateway *gw)
{
  ...
  char payload[1024];
  size_t ret = json_inject(payload,
                           sizeof(payload),
                           "(op):6" /* RESUME OPCODE */
                           "(d):{"
                           "(token):s"
...
on_hello(struct discord_gateway *gw)
{
  ...
  json_extract(gw->payload->event_data.start,
               gw->payload->event_data.size,
               "(heartbeat_interval):ld",
               &gw->hbeat->interval_ms);
...
#define RETURN_IF_MATCH(event, str)                                           \
  if (STREQ(#event, str))                                                     \
  return DISCORD_GATEWAY_EVENTS_##event
...
on_message_create(struct discord_gateway *gw, struct sized_buffer *data)
{
  ...
  if (gw->user_cmd->pool && STRNEQ(gw->user_cmd->prefix.start,
                                   msg->content,
  ...
  if (gw->user_cmd->cbs.sb_on_message_create) /* @todo temporary */
    (*gw->user_cmd->cbs.sb_on_message_create)(
      CLIENT(gw), &gw->bot, &gw->sb_bot, msg, data);
                           ...
                           struct sized_buffer *data)
{
  ...
  ON(
    message_reaction_remove, user_id, channel_id, message_id, guild_id, emoji);
...
dispatch_run(void *p_cxt)
{
  ...
  if (cxt->is_main_thread) {
    ...
    (*cxt->p_gw->user_cmd->cbs.on_event_raw)(
      CLIENT(cxt->p_gw), cxt->event, &cxt->p_gw->sb_bot, &cxt->data);
    return;
  }
...
on_dispatch(struct discord_gateway *gw)
{
  ...
  if (!on_event)
    return; /* user not subscribed to the event */

  enum discord_event_scheduler mode;
  mode = gw->user_cmd->scheduler(
    CLIENT(gw), &gw->bot, &gw->payload->event_data, event);
  switch (mode) {
...
{
  gw->status->is_resumable = strncmp(
    gw->payload->event_data.start, "false", gw->payload->event_data.size);
...
static void
on_connect_cb(void *p_gw, struct websockets *ws, struct ws_info *info,
...
static void
on_close_cb(void *p_gw, struct websockets *ws, struct ws_info *info,
...
static void
on_text_cb(void *p_gw, struct websockets *ws, struct ws_info *info,
...
static void
noop_event_raw_cb(struct discord *a, enum discord_gateway_events b,
...
void
discord_gateway_init(struct discord_gateway *gw, struct logconf *conf,
                     struct sized_buffer *token)
{
  ...
  *gw->id.properties = (struct discord_identify_connection){
  ...
  discord_set_presence(
    CLIENT(gw),
    &(struct discord_presence_status){ .activities = NULL,
                                       .status = "online",
                                       .afk = false,
