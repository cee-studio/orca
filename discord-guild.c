...
ORCAcode
discord_get_guild(struct discord *client, const u64_snowflake_t guild_id,
                  struct discord_guild *p_guild)
{
  if (!guild_id) {
                          ...
                          struct discord_guild_preview *p_guild_preview)
{
  ...
  return discord_adapter_run(
    ...
    NULL,
    HTTP_GET,
    "/guilds/%" PRIu64 "/preview",
    guild_id);
}

ORCAcode
discord_modify_guild(struct discord *client, const u64_snowflake_t guild_id,
...
discord_delete_guild(struct discord *client, const u64_snowflake_t guild_id)
{
  ...
  return discord_adapter_run(
    &client->adapter, NULL, NULL, HTTP_DELETE, "/guilds/%" PRIu64, guild_id);
}
...
ORCAcode
discord_get_guild_member(struct discord *client, u64_snowflake_t guild_id,
                           ...
                           NTL_T(struct discord_guild_member) * p_members)
{
  ...
  if (params) {
    ...
    if (params->limit) {
      offset += snprintf(
        query + offset, sizeof(query) - offset, "limit=%d", params->limit);
      ASSERT_S(offset < sizeof(query), "Out of bounds write attempt");
    }
    if (params->after) {
      offset += snprintf(query + offset,
                         sizeof(query) - offset,
                         "%safter=%" PRIu64,
                         (*query) ? "&" : "",
                         params->after);
...
ORCAcode
discord_get_guild_bans(struct discord *client, const u64_snowflake_t guild_id,
...
ORCAcode
discord_get_guild_ban(struct discord *client, const u64_snowflake_t guild_id,
                      const u64_snowflake_t user_id, struct discord_ban *p_ban)
{
  if (!guild_id) {
    log_error("Missing 'guild_id'");
    return ORCA_MISSING_PARAMETER;
...
ORCAcode
discord_get_guild_roles(struct discord *client, const u64_snowflake_t guild_id,
                          ...
                          struct discord_role *p_role)
{
  ...
  size_t ret;
  if (params)
    ret = discord_modify_guild_role_params_to_json(
      payload, sizeof(payload), params);
  else
    ret = sprintf(payload, "{}");
                          ...
                          struct discord_begin_guild_prune_params *params)
{
  ...
  if (params)
    ret = discord_begin_guild_prune_params_to_json(
      payload, sizeof(payload), params);
  else
    ret = sprintf(payload, "{}");
