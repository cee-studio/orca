 ...
 */
void discord_adapter_init(struct discord_adapter* adapter, struct logconf* conf,
...
void discord_bucket_build(struct discord_adapter* adapter,
                          ...
                          ORCAcode code, struct ua_info* info);
...
struct discord_gateway_cbs {
  ...
  discord_event_raw_cb
          on_event_raw; /**< triggers for every event if set, receive its raw JSON string */
  ...
  discord_channel_pins_update_cb
          on_channel_pins_update; /**< triggers when a channel pinned messages updates */
  ...
  discord_guild_member_remove_cb
          on_guild_member_remove; /**< triggers when a guild member is removed from a guild */
  discord_guild_member_cb
          on_guild_member_update; /**< triggers when a guild member status is updated (ex: receive role) */
  ...
  discord_message_reaction_add_cb
          on_message_reaction_add; /**< triggers when a reaction is added to a message */
  discord_message_reaction_remove_cb
          on_message_reaction_remove; /**< triggers when a reaction is removed from a message */
  discord_message_reaction_remove_all_cb
          on_message_reaction_remove_all; /**< triggers when all reactions are removed from a message */
  discord_message_reaction_remove_emoji_cb
          on_message_reaction_remove_emoji; /**< triggers when all occurences of a specific reaction is removed from a message */
...
struct discord_gateway {
  ...
  struct {
    ...
    u64_unix_ms_t
            event_tstamp; /**< timestamp of last succesful event timestamp in ms (resets every 60s) */
  ...
  struct { /**< User-Commands structure */
    struct sized_buffer
            prefix; /**< the prefix expected before every command @see discord_set_prefix() */
    struct discord_gateway_cmd_cbs*
            pool; /**< user's command/callback pair @see discord_set_on_command() */
    size_t amt; /**< amount of command/callback pairs in pool */
    struct discord_gateway_cmd_cbs
            on_default; /**< user's default callback incase prefix matches but command doesn't */

    struct discord_gateway_cbs cbs; /**< user's callbacks */
    discord_event_scheduler_cb
            scheduler; /**< context on how each event callback is executed @see discord_set_event_scheduler() */
  } * user_cmd;
};
...
struct discord {
  ...
  struct discord_voice
          vcs[DISCORD_MAX_VOICE_CONNECTIONS]; /**< the WebSockets handles for establishing voice connections to Discord */
