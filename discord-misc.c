...
_discord_params_to_mime(curl_mime *mime, void *p_cxt)
{
  ...
  for (int i = 0; atchs[i]; ++i) {
    ...
    if (atchs[i]->content) {
      ...
      curl_mime_filename(
        part,
        IS_EMPTY_STRING(atchs[i]->filename) ? "a.out" : atchs[i]->filename);
      curl_mime_type(part,
                     IS_EMPTY_STRING(atchs[i]->content_type)
                       ? "application/octet-stream"
                       : atchs[i]->content_type);
      curl_mime_name(part, name);
    }
    else if (!IS_EMPTY_STRING(
               atchs[i]->filename)) { /* fetch local file by the filename */
      part = curl_mime_addpart(mime);
...
void
discord_embed_set_footer(struct discord_embed *embed, char text[],
...
void
discord_embed_set_thumbnail(struct discord_embed *embed, char url[],
...
void
discord_embed_set_image(struct discord_embed *embed, char url[],
                        char proxy_url[], int height, int width)
{
  if (embed->image)
...
void
discord_embed_set_video(struct discord_embed *embed, char url[],
                        char proxy_url[], int height, int width)
{
  if (embed->video)
...
void
discord_embed_set_provider(struct discord_embed *embed, char name[],
                           char url[])
{
  if (embed->provider)
...
void
discord_embed_set_author(struct discord_embed *embed, char name[], char url[],
                         char icon_url[], char proxy_icon_url[])
{
  if (embed->author)
...
void
discord_embed_add_field(struct discord_embed *embed, char name[], char value[],
                        bool Inline)
{
  ...
  ntl_append2(
    (ntl_t *)&embed->fields, sizeof(struct discord_embed_field), &field);
}
...
{
  ntl_append2(
    (ntl_t *)&presence->activities, sizeof(struct discord_activity), activity);
}
