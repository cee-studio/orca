...
bucket_init(struct sized_buffer *hash, const char route[])
{
  ...
  int ret = snprintf(new_bucket->hash,
                     sizeof(new_bucket->hash),
                     "%.*s",
                     (int)hash->size,
                     hash->start);
...
discord_bucket_try_get(struct discord_adapter *adapter, const char route[])
{
  logconf_debug(&adapter->ratelimit->conf,
                "[?] Attempt to find matching bucket for route '%s'",
                route);
  ...
  if (!bucket)
    ...
    logconf_debug(
      &adapter->ratelimit->conf, "[%.4s] Found a match!", bucket->hash);
                 ...
                 struct ua_info *info)
{
  if (code != ORCA_OK) {
    logconf_debug(
      &adapter->ratelimit->conf, "[%.4s] Request failed", bucket->hash);
  }
 ...
 *  route*/
static void
match_route(struct discord_adapter *adapter, const char route[], ORCAcode code,
            struct ua_info *info)
{
  ...
  logconf_debug(&adapter->ratelimit->conf,
                "[%.4s] Assign new route '%s' to bucket",
                bucket->hash,
                bucket->route);
