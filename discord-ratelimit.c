...
static struct discord_bucket* bucket_init(struct sized_buffer* hash,
                                          const char route[])
{
...
static void bucket_cleanup(struct discord_bucket* bucket)
{
                                              ...
                                              const char route[])
{
  ...
  if (!bucket)
    logconf_debug(
            &adapter->ratelimit->conf,
            "[?] Couldn't match bucket to route '%s', will attempt to create a new one",
            route);
  else
...
static void parse_ratelimits(struct discord_adapter* adapter,
                             struct discord_bucket* bucket, ORCAcode code,
                             struct ua_info* info)
{
  if (code != ORCA_OK) {
    ...
    struct sized_buffer reset = ua_info_header_get(info, "x-ratelimit-reset"),
                        remaining = ua_info_header_get(info,
                                                       "x-ratelimit-remaining"),
                        reset_after = ua_info_header_get(
    ...
    if (reset.size)
      ...
      u64_unix_ms_t now_tstamp = date.size ?
                                         1000 * curl_getdate(date.start, NULL) :
                                         cee_timestamp_ms();
      bucket->reset_tstamp =
 ...
 *  route*/
static void match_route(struct discord_adapter* adapter, const char route[],
