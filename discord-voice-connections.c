...
on_hello(struct discord_voice *vc)
{
  ...
  json_extract(vc->payload.event_data.start,
...
{
  struct discord *client = vc->p_client;
  if (!client->voice_cbs.on_speaking)
    return;
  ...
  (*client->voice_cbs.on_speaking)(
    client, vc, &vc->p_client->gw.bot, user_id, speaking, delay, ssrc);
}
...
on_client_disconnect(struct discord_voice *vc)
{
  ...
  if (!client->voice_cbs.on_client_disconnect)
    return;
  ...
  (*client->voice_cbs.on_client_disconnect)(
    client, vc, &client->gw.bot, user_id);
}
...
on_codec(struct discord_voice *vc)
{
  ...
  if (!client->voice_cbs.on_codec)
    return;

  char audio_codec[64] = { 0 }, video_codec[64] = { 0 };
  json_extract(vc->payload.event_data.start,
               vc->payload.event_data.size,
               "(audio_codec):s, (video_codec):s",
               &audio_codec,
               &video_codec);

  (*client->voice_cbs.on_codec)(
    client, vc, &client->gw.bot, audio_codec, video_codec);
}
...
static void
on_connect_cb(void *p_vc, struct websockets *ws, struct ws_info *info,
...
static void
on_close_cb(void *p_vc, struct websockets *ws, struct ws_info *info,
...
static void
on_text_cb(void *p_vc, struct websockets *ws, struct ws_info *info,
           const char *text, size_t len)
{
  struct discord_voice *vc = p_vc;

  json_extract((char *)text,
               len,
               "(op):d (d):T",
               &vc->payload.opcode,
  ...
  default:
    logconf_error(&vc->conf,
                  "Not yet implemented Voice Event(code: %d)",
                  vc->payload.opcode);
    break;
  }
}
...
send_heartbeat(struct discord_voice *vc)
{
  char payload[64];
  int ret = json_inject(
    payload, sizeof(payload), "(op):3, (d):ld", &vc->hbeat.interval_ms);
  ASSERT_S(ret < sizeof(payload), "Out of bounds write attempt");
...
static void
_discord_voice_init(struct discord_voice *new_vc, struct discord *client,
                    u64_snowflake_t guild_id, u64_snowflake_t channel_id)
{
...
static void
recycle_active_vc(struct discord_voice *vc, u64_snowflake_t guild_id,
...
static void
send_voice_state_update(struct discord_voice *vc, u64_snowflake_t guild_id,
...
discord_voice_join(struct discord *client, u64_snowflake_t guild_id,
                   u64_snowflake_t vchannel_id, bool self_mute, bool self_deaf)
{
                               ...
                               struct discord_voice_state *vs)
{
  ...
  for (i = 0; i < DISCORD_MAX_VOICE_CONNECTIONS; ++i) {
    if (vs->guild_id == client->vcs[i].guild_id) {
      ...
      if (vs->channel_id) {
        int ret = snprintf(
          vc->session_id, sizeof(vc->session_id), "%s", vs->session_id);
  ...
  if (vs->channel_id == 0) {
    logconf_info(
      &vc->conf,
      ANSICOLOR("Bot is leaving the current vc", ANSI_BG_BRIGHT_BLUE));
                                ...
                                char *endpoint)
{
  ...
  ret = snprintf(vc->new_url,
                 sizeof(vc->new_url),
                 "wss://%s" DISCORD_VOICE_CONNECTIONS_URL_SUFFIX,
                 endpoint);
...
static void
noop_voice_state_update_cb(struct discord *a, const struct discord_user *b,
                           const struct discord_voice_state *c)
{
  return;
}
static void
noop_voice_server_update_cb(struct discord *a, const struct discord_user *b,
...
static void
noop_idle_cb(struct discord *a, struct discord_voice *b,
             const struct discord_user *c)
{
  return;
}
static void
noop_on_speaking(struct discord *a, struct discord_voice *b,
                 const struct discord_user *c, const u64_snowflake_t d,
                 const int e, const int f, const int g)
{
  return;
}
static void
noop_on_voice_client_disconnect(struct discord *a, struct discord_voice *b,
                                const struct discord_user *c,
...
static void
noop_on_voice_codec(struct discord *a, struct discord_voice *b,
