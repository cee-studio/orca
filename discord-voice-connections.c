...
static const char* opcode_print(enum discord_voice_opcodes opcode)
{
...
static void send_resume(struct discord_voice* vc)
{
  ...
  logconf_info(&vc->conf,
               ANSICOLOR("SEND",
                         ANSI_FG_BRIGHT_GREEN) " VOICE_RESUME (%d bytes)",
               ret);
  ws_send_text(vc->ws, NULL, payload, ret);
}

static void send_identify(struct discord_voice* vc)
{
  char payload[1024];
  int ret = json_inject(payload, sizeof(payload),
  ...
  logconf_info(&vc->conf,
               ANSICOLOR("SEND",
                         ANSI_FG_BRIGHT_GREEN) " VOICE_IDENTIFY (%d bytes)",
               ret);
  ws_send_text(vc->ws, NULL, payload, ret);
}

static void on_hello(struct discord_voice* vc)
{
...
static void on_ready(struct discord_voice* vc)
{
...
static void on_session_description(struct discord_voice* vc)
{
  struct discord* client = vc->p_client;
...
static void on_speaking(struct discord_voice* vc)
{
  struct discord* client = vc->p_client;
...
static void on_resumed(struct discord_voice* vc)
{
  vc->is_ready = true;
  vc->reconnect.attempt = 0;
...
static void on_client_disconnect(struct discord_voice* vc)
{
  struct discord* client = vc->p_client;
...
static void on_codec(struct discord_voice* vc)
{
  struct discord* client = vc->p_client;
...
static void on_heartbeat_ack(struct discord_voice* vc)
{
...
static void on_connect_cb(void* p_vc, struct websockets* ws,
...
static void on_close_cb(void* p_vc, struct websockets* ws, struct ws_info* info,
...
static void on_text_cb(void* p_vc, struct websockets* ws, struct ws_info* info,
...
static void send_heartbeat(struct discord_voice* vc)
{
  char payload[64];
  ...
  logconf_info(&vc->conf,
               ANSICOLOR("SEND",
                         ANSI_FG_BRIGHT_GREEN) " VOICE_HEARTBEAT (%d bytes)",
               ret);
  ws_send_text(vc->ws, NULL, payload, ret);
}

static void _discord_voice_cleanup(struct discord_voice* vc)
{
...
static void reset_vc(struct discord_voice* vc)
{
  vc->reconnect.attempt = 0;
  vc->shutdown = false;
}

static void _discord_voice_init(struct discord_voice* new_vc,
...
void discord_send_speaking(struct discord_voice* vc,
                           enum discord_voice_speaking_flags flag, int delay)
{
  ...
  logconf_info(&vc->conf,
               ANSICOLOR("SEND",
                         ANSI_FG_BRIGHT_GREEN) " VOICE_SPEAKING (%d bytes)",
               ret);
  ws_send_text(vc->ws, NULL, payload, ret);
}

static void recycle_active_vc(struct discord_voice* vc,
                              u64_snowflake_t guild_id,
                              u64_snowflake_t channel_id)
{
...
static void send_voice_state_update(struct discord_voice* vc,
                                    u64_snowflake_t guild_id,
                                    u64_snowflake_t channel_id, bool self_mute,
                                    bool self_deaf)
{
...
static void event_loop(struct discord_voice* vc)
{
  struct discord* client = vc->p_client;
  ws_start(vc->ws);
...
static void* start_voice_ws_thread(void* p_vc)
{
  struct discord_voice* vc = p_vc;
...
void _discord_on_voice_server_update(struct discord* client,
                                     u64_snowflake_t guild_id, char* token,
                                     char* endpoint)
{
  struct discord_voice* vc = NULL;
  pthread_mutex_lock(&client_lock);
...
static void noop_voice_state_update_cb(struct discord* a,
                                       const struct discord_user* b,
                                       const struct discord_voice_state* c)
{
  return;
}
static void noop_voice_server_update_cb(struct discord* a,
                                        const struct discord_user* b,
                                        const char* c, const u64_snowflake_t d,
                                        const char* endpoint)
{
  return;
}
static void noop_idle_cb(struct discord* a, struct discord_voice* b,
                         const struct discord_user* c)
{
  return;
}
static void noop_on_speaking(struct discord* a, struct discord_voice* b,
                             const struct discord_user* c,
                             const u64_snowflake_t d, const int e, const int f,
                             const int g)
{
  return;
}
static void noop_on_voice_client_disconnect(struct discord* a,
...
static void noop_on_voice_codec(struct discord* a, struct discord_voice* b,
                                const struct discord_user* c, const char d[],
                                const char e[])
{
  return;
}
static void noop_on_ready(struct discord_voice* a)
{
  return;
}
static void noop_on_session_descriptor(struct discord_voice* a)
{
  return;
}
static void noop_on_udp_server_connected(struct discord_voice* a)
{
  return;
}
