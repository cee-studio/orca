...
{
  ASSERT_S(
    WS_CONNECTED == ws_get_status(vc->ws),
  ...
  bool self_deaf)
{
  ...
  if (!vc) {
    logconf_error(
      client->conf, "All VC are busy, cannot send VOICE_STATE_UPDATE");
...
static void
noop_idle_cb(
  struct discord *a, struct discord_voice *b, const struct discord_user *c)
{
  return;
}
static void
...
static void
noop_on_voice_client_disconnect(
  struct discord *a,
  struct discord_voice *b,
  const struct discord_user *c,
  const u64_snowflake_t d)
