...
struct discord_voice_cbs { /* CALLBACKS STRUCTURE */
  ...
  discord_voice_client_disconnect_cb
    on_client_disconnect; /**< triggers when a user has disconnected from the
                             voice channel */
...
struct discord_voice {
  ...
  u64_snowflake_t guild_id; /**< the session guild id @note obtained from
                               discord_voice_join() */
  u64_snowflake_t channel_id; /**< the session channel id @note obtained from
                                 discord_voice_join() */
  /** @note obtained from on_voice_server_update() */
  char token[128]; /**< the session token @note obtained from
                      on_voice_server_update() */
  char new_token[128]; /**< the new session token after a voice region change
                          @note obtained from on_voice_server_update() */
  char new_url[512]; /**< the new url after a voice region change @note
                        obtained from on_voice_server_update() */

  /** @note obtained from on_voice_state_update() */
  char session_id[128]; /**< the session id @note obtained from
                           on_voice_state_update() */
  u64_snowflake_t
    bot_id; /**< the bot user id @note obtained from on_voice_state_update() */
  struct websockets
  ...
  bool
    is_ready; /**< can start sending/receiving additional events to discord */
  ...
  int ping_ms; /**< latency between client and websockets server, calculated by
                  the interval between HEARTBEAT and HEARTBEAT_ACK */
 ...
 */
void discord_voice_set_on_idle(
  struct discord_voice *vc, discord_voice_idle_cb *callback);
 ...
 */
enum discord_voice_status discord_voice_join(
  struct discord *client,
 ...
 */
void discord_send_speaking(
  struct discord_voice *vc, enum discord_voice_speaking_flags flag, int delay);
 ...
 */
void _discord_on_voice_state_update(
  struct discord *client, struct discord_voice_state *vs);
