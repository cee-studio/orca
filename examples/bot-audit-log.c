...
void
on_log_guild_member_add(struct discord *client, const struct discord_user *bot,
                        const uint64_t guild_id,
                        const struct discord_guild_member *member)
{
  log_info("%s#%s joined guild %" PRIu64,
...
{
  log_info("%s#%s left guild %" PRIu64,
           user->username,
           user->discriminator,
           guild_id);
}
...
void
on_audit_channel_create(struct discord *client, const struct discord_user *bot,
