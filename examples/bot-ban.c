...
void
on_guild_ban_add(struct discord *client, const struct discord_user *bot,
                 const u64_snowflake_t guild_id,
                 const struct discord_user *user)
{
  struct discord_channel general = { 0 };
  if (discord_get_channel_at_pos(
        client, guild_id, DISCORD_CHANNEL_GUILD_TEXT, 0, &general)) {
...
void
on_guild_ban_remove(struct discord *client, const struct discord_user *bot,
                    const u64_snowflake_t guild_id,
                    const struct discord_user *user)
{
  struct discord_channel general = { 0 };
...
void
on_ban(struct discord *client, const struct discord_user *bot,
       const struct discord_message *msg)
{
  ...
  if (!target)
    return; // member is not in guild

  char reason[128];
  snprintf(reason, sizeof(reason), "%s said so", msg->author->username);
  discord_create_guild_ban(client,
                           msg->guild_id,
                           target->id,
...
void
on_unban(struct discord *client, const struct discord_user *bot,
         const struct discord_message *msg)
{
  // get banned list
