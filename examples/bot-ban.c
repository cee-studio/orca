...
void on_guild_ban_add(struct discord *client, const struct discord_user *bot,
...
void on_guild_ban_remove(struct discord *client, const struct discord_user *bot,
                         const u64_snowflake_t guild_id,
                         const struct discord_user *user)
{
  struct discord_channel general = { 0 };
...
void on_ban(struct discord *client, const struct discord_user *bot,
            const struct discord_message *msg)
{
  ...
  ORCAcode code = discord_list_guild_members(
          client, msg->guild_id,
          &(struct discord_list_guild_members_params){ .limit = 1000,
                                                       .after = 0 },
          &members);
  ...
  discord_create_guild_ban(
          client, msg->guild_id, target->id,
          &(struct discord_create_guild_ban_params){ .delete_message_days = 1,
                                                     .reason = reason });

  discord_guild_member_list_free(members);
}

void on_unban(struct discord *client, const struct discord_user *bot,
              const struct discord_message *msg)
{
  // get banned list
