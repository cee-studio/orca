...
void
on_from_json_init(struct discord *client, const struct discord_user *bot,
                  const struct discord_message *msg)
{
  ...
  discord_component_list_from_json(
    JSON_STRING, sizeof(JSON_STRING), &components);
...
void
on_designated_init(struct discord *client, const struct discord_user *bot,
                   const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
...
void
on_dynamic_init(struct discord *client, const struct discord_user *bot,
                const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
  ...
  *option.emoji = (struct discord_emoji){ .name = strdup("rogue"),
                                          .id = 625891304148303894ULL };
  ntl_append2(
    (ntl_t *)&select.options, sizeof(struct discord_select_option), &option);
  ...
  *option.emoji = (struct discord_emoji){ .name = strdup("priest"),
  ...
  ntl_append2(
    (ntl_t *)&action.components, sizeof(struct discord_component), &select);
...
void
on_interaction_create(struct discord *client, const struct discord_user *bot,
