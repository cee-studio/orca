...
void on_from_json_init(struct discord* client, const struct discord_user* bot,
...
void on_designated_init(struct discord* client, const struct discord_user* bot,
                        const struct discord_message* msg)
{
  if (msg->author->bot) return;
                                    ...
                                    &(struct discord_component){
                                            ...
                                            .options =
                                                    (struct discord_select_option*[]){
                                                            ...
                                                            &(struct
                                                              discord_select_option){ .label = "Mage", .value = "mage", .description = "Turn 'em into a sheep", .emoji = &(struct discord_emoji){ .name = "mage", .id = 625891304081063986ULL } },
                                                            &(struct
                                                              discord_select_option){ .label =
                                                                                              "Priest",
                                                                                      .value = "priest",
                                                                                      .description =
                                                                                              "You get heals when I'm done doing damage",
                                                                                      .emoji = &(struct discord_emoji){ .name = "priest", .id = 625891303795982337ULL } },
...
void on_dynamic_init(struct discord* client, const struct discord_user* bot,
                     const struct discord_message* msg)
{
  if (msg->author->bot) return;
  ...
  struct discord_interaction_response params = {
    ...
    .data =
            &(struct discord_interaction_callback_data){
                    .content = text,
                    .flags =
                            DISCORD_INTERACTION_CALLBACK_DATA_EPHEMERAL // 1 << 6
            }
  };

  ORCAcode code;
  code = discord_create_interaction_response(client, interaction->id,
