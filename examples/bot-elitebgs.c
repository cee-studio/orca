  ...
  for (size_t i = 0; l_docs[i]; ++i) {
    ...
    for (size_t i = 0; l_fpresence[i]; ++i) {
      ...
      int ret = snprintf(field_value, sizeof(field_value),
                         ...
                         "Influence: %.1f%s%.1f\n"
                         "Happiness: %s\n",
                         fpresence->state, fpresence->influence * 100,
                         influence_emoji, influence_diff,
      ...
      if (*l_recovering_states) {
        for (size_t i = 0; l_recovering_states[i]; ++i) {
          json_extract(l_recovering_states[i]->start,
                       ...
                       "(state):.*s"
                       "(trend):.*s",
                       sizeof(state->state), state->state, sizeof(state->trend),
                       state->trend);
          ret += snprintf(&field_value[ret], sizeof(field_value) - ret, " %s,",
                          state->state);
          //@todo use trend
...
void on_command(struct discord *client, const struct discord_user *bot,
