...
void
ticks_from_json(char *str, size_t len, void *p_tick_ms)
{
...
void
update_last_tick_ms(uint64_t *tick_ms)
{
...
char *
happiness_localised(char *happiness_band)
{
...
void
embed_from_json(char *str, size_t len, void *p_embed)
{
  ...
  for (size_t i = 0; l_docs[i]; ++i) {
    json_extract(l_docs[i]->start,
                 ...
                 sizeof(doc->name),
                 doc->name,
                 sizeof(doc->government),
                 doc->government,
                 &l_fpresence,
                 sizeof(doc->updated_at),
                 doc->updated_at,
                 &l_history);

    for (size_t i = 0; l_fpresence[i]; ++i) {
      json_extract(l_fpresence[i]->start,
                   l_fpresence[i]->size,
                   "(system_name):.*s"
                   "(system_id):.*s"
                   "(state):.*s"
                   ...
                   "(recovering_states):[L]"
                   "(updated_at):.*s",
                   sizeof(fpresence->system_name),
                   fpresence->system_name,
                   sizeof(fpresence->system_id),
                   fpresence->system_id,
                   sizeof(fpresence->state),
                   fpresence->state,
                   &fpresence->influence,
                   sizeof(fpresence->happiness),
                   fpresence->happiness,
                   &l_active_states,
                   &l_pending_states,
                   &l_recovering_states,
                   sizeof(fpresence->updated_at),
      ...
      for (size_t i = 0; l_history[i]; ++i) {
        json_extract(l_history[i]->start,
                     l_history[i]->size,
                     "(system_id):.*s"
                     "(influence):f"
                     "(updated_at):.*s",
                     sizeof(history->system_id),
                     history->system_id,
                     &history->influence,
                     sizeof(history->system_id),
                     history->updated_at);
      ...
      int ret = snprintf(field_value,
                         sizeof(field_value),
                         "State: %s\n"
                         "Influence: %.1f%s%.1f\n"
                         "Happiness: %s\n",
                         fpresence->state,
                         fpresence->influence * 100,
                         influence_emoji,
                         influence_diff,
                         happiness_localised(fpresence->happiness));

      ret += snprintf(
        &field_value[ret], sizeof(field_value) - ret, "Active States:");

      if (!*l_active_states) {
        for (size_t i = 0; l_active_states[i]; ++i) {
          json_extract(l_active_states[i]->start,
                       l_active_states[i]->size,
                       "(state):.*s",
                       sizeof(state->state),
                       state->state);
          ret += snprintf(&field_value[ret],
                          sizeof(field_value) - ret,
                          " %s,",
                          state->state);
        }
      ...
      ret += snprintf(
        &field_value[ret], sizeof(field_value) - ret, "Pending States:");
      if (*l_pending_states) {
        for (size_t i = 0; l_pending_states[i]; ++i) {
          json_extract(l_pending_states[i]->start,
                       l_pending_states[i]->size,
                       "(state):.*s",
                       sizeof(state->state),
                       state->state);
          ret += snprintf(&field_value[ret],
      ...
      ret += snprintf(
        &field_value[ret], sizeof(field_value) - ret, "Recovering States:");
...
void
on_command(struct discord *client, const struct discord_user *bot,
