...
void fetch_member_msgs(
  struct discord *client, u64_snowflake_t guild_id, u64_snowflake_t user_id)
{
  ...
  for (int i = 0; channels[i]; ++i) {
    ...
    do {
      discord_get_channel_messages(
        client, channels[i]->id, &params, &messages);
      if (!messages) break; /* EARLY BREAK */

      for (n_msg = 0; messages[n_msg]; ++n_msg) {
        if (
          user_id == messages[n_msg]->author->id &&
