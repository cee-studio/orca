...
void
on_bot_get_guild_template(struct discord *client,
...
void
on_bot_create_guild_template(struct discord *client,
                             const struct discord_user *bot,
                             const struct discord_message *message)
{
  char message_buffer[8001];
  ...
  ORCAcode response = discord_create_guild_template(
    client, message->guild_id, &params, &template);

  if (response == ORCA_OK) {
    snprintf(message_buffer,
             8001,
             "Here is some information about your new guild template:\nName: "
             "'%s'\nDescription: '%s'\nCreator Id: %" PRIu64 "\n",
             template.name,
             template.description,
             template.creator_id);
  ...
  discord_create_message(
    client, message->channel_id, &message_params, &p_message);
