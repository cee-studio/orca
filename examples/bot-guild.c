...
void on_log_role_create(struct discord* client, const struct discord_user* bot,
...
void on_log_role_update(struct discord* client, const struct discord_user* bot,
                        const u64_snowflake_t guild_id,
                        const struct discord_role* role)
{
...
void on_log_role_delete(struct discord* client, const struct discord_user* bot,
                        const u64_snowflake_t guild_id,
               ...
               const struct discord_message* msg)
{
  ...
  ORCAcode code;
  code = discord_create_guild(client,
                              &(struct discord_create_guild_params){
                                      .name = *msg->content ? msg->content :
                                                              "TestGuild" },
                              &guild);
...
void on_modify(struct discord* client, const struct discord_user* bot,
               const struct discord_message* msg)
{
  if (msg->author->bot) return;
...
void on_role_create(struct discord* client, const struct discord_user* bot,
                    const struct discord_message* msg)
{
  if (msg->author->bot) return;
...
void on_role_delete(struct discord* client, const struct discord_user* bot,
                    const struct discord_message* msg)
{
  if (msg->author->bot) return;
...
void on_role_member_add(struct discord* client, const struct discord_user* bot,
                        const struct discord_message* msg)
{
  if (msg->author->bot) return;
                           ...
                           const struct discord_message* msg)
{
  ...
  if (!user_id || !role_id) {
    ...
    if (ORCA_OK == discord_remove_guild_member_role(client, msg->guild_id,
                                                    user_id, role_id))
      sprintf(text, "Removed role <@&%" PRIu64 "> from <@%" PRIu64 ">", role_id,
              user_id);
    else
...
void on_role_list(struct discord* client, const struct discord_user* bot,
                  const struct discord_message* msg)
{
  if (msg->author->bot) return;
...
void on_member_get(struct discord* client, const struct discord_user* bot,
                   const struct discord_message* msg)
{
  if (msg->author->bot) return;
...
void on_member_search(struct discord* client, const struct discord_user* bot,
                      const struct discord_message* msg)
{
  if (msg->author->bot) return;
  ...
  code = discord_search_guild_members(
          client, msg->guild_id,
          &(struct discord_search_guild_members_params){ .query =
                                                                 msg->content },
          &members);
...
void on_bot_change_nick(struct discord* client, const struct discord_user* bot,
                        const struct discord_message* msg)
{
  if (msg->author->bot) return;
  ...
  if (!*msg->content) {
    ...
    if (ORCA_OK ==
        discord_modify_current_user_nick(client, msg->guild_id, &params, NULL))
...
void on_bot_get_invites(struct discord* client, const struct discord_user* bot,
                        const struct discord_message* msg)
{
  if (msg->author->bot) return;
