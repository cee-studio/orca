...
void
on_log_role_create(struct discord *client, const struct discord_user *bot,
...
void
on_log_role_update(struct discord *client, const struct discord_user *bot,
                   const u64_snowflake_t guild_id,
                   const struct discord_role *role)
{
...
void
on_log_role_delete(struct discord *client, const struct discord_user *bot,
                   const u64_snowflake_t guild_id,
...
void
on_create(struct discord *client, const struct discord_user *bot,
...
void
on_modify(struct discord *client, const struct discord_user *bot,
          const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
...
void
on_delete(struct discord *client, const struct discord_user *bot,
          const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
...
void
on_role_create(struct discord *client, const struct discord_user *bot,
               const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
...
void
on_role_delete(struct discord *client, const struct discord_user *bot,
               const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
...
void
on_role_member_add(struct discord *client, const struct discord_user *bot,
                   const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
  ...
  if (!user_id || !role_id) {
    ...
    if (ORCA_OK ==
      ...
      sprintf(text,
              "Assigned role <@&%" PRIu64 "> to <@%" PRIu64 ">",
              role_id,
              user_id);
    else
      sprintf(text,
...
void
on_role_member_remove(struct discord *client, const struct discord_user *bot,
                      const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
  ...
  if (!user_id || !role_id) {
    ...
    if (ORCA_OK == discord_remove_guild_member_role(
                     client, msg->guild_id, user_id, role_id))
      sprintf(text,
              "Removed role <@&%" PRIu64 "> from <@%" PRIu64 ">",
              role_id,
              user_id);
    else
      sprintf(text,
...
void
on_role_list(struct discord *client, const struct discord_user *bot,
             const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
...
void
on_member_get(struct discord *client, const struct discord_user *bot,
              const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
  ...
  if (!user_id) {
    ...
    if (ORCA_OK == discord_get_guild_member(
                     client, msg->guild_id, msg->author->id, &member))
...
void
on_member_change_nick(struct discord *client, const struct discord_user *bot,
                      const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
  ...
  if (!user_id || !*nick) {
    ...
    if (ORCA_OK == discord_modify_guild_member(
                     client, msg->guild_id, user_id, &params, NULL))
...
void
on_member_search(struct discord *client, const struct discord_user *bot,
                 const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
...
void
on_bot_change_nick(struct discord *client, const struct discord_user *bot,
                   const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
                          ...
                          const struct discord_message *msg)
{
  ...
  if (response != ORCA_OK) {
    sprintf(
      text, "Could not fetch welcome screen from guild %s.", msg->content);
  }
  else {
    sprintf(text, "%s\n", screen.description);
  }
...
void
on_bot_get_invites(struct discord *client, const struct discord_user *bot,
                   const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
