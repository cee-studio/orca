...
{
  log_info("Quiz-Bot succesfully connected to Discord as %s#%s!", bot->username,
           bot->discriminator);
}
...
void start_new_session(struct discord* client, const u64_snowflake_t guild_id,
                       const struct discord_guild_member* member)
{
...
void send_next_question(struct discord* client, u64_snowflake_t channel_id,
                        struct session* session, struct question* question)
{
  ...
  if (session->curr_question == g_session.questions_per_session) {
    sprintf(text, "You got %d out of %d! (%.1f%%)", session->hits,
            ...
            100 * ((float)session->hits /
                   (float)g_session.questions_per_session));
  ...
  for (int i = 0; i < question->num_answers; ++i) {
    offset += sprintf(text + offset, "(%c)%s ", 'A' + i,
                      question->answers[i].desc);
  }
...
void on_reaction_add(struct discord* client, const struct discord_user* bot,
                     ...
                     const struct discord_emoji* emoji)
{
  ...
  if ((message_id == g_session.message_id) &&
      (0 ==
       strcmp(emoji->name,
              g_session.reaction_emoji))) { // close existing quiz session / start new quiz session
...
int main(int argc, char* argv[])
{
  ...
  discord_create_reaction(client, g_session.channel_id, g_session.message_id, 0,
                          g_session.reaction_emoji);
