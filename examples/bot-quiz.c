...
parse_session_config()
{
  ...
  for (size_t i = 0; t_questions[i]; ++i) {
    ...
    json_extract(t_questions[i]->start,
                 t_questions[i]->size,
                 "(description):?s",
                 &g_session.questions[i].desc);
    json_extract(t_questions[i]->start,
                 t_questions[i]->size,
                 "(answers):[L]",
                 &t_answers);
    ...
    for (int j = 0; t_answers[j]; ++j) {
      json_extract(t_answers[j]->start,
                   t_answers[j]->size,
                   "(description):?s"
                   "(value):b",
 ...
 * the user */
void
close_existing_sessions(struct discord *client, const u64_snowflake_t guild_id,
...
u64_snowflake_t
create_session_channel(struct discord *client, const u64_snowflake_t guild_id,
                       const struct discord_guild_member *member)
{
  ...
  discord_overwrite_append(&params1.permission_overwrites,
                           member->user->id,
                           1, // user type
...
u64_snowflake_t
add_session_role(struct discord *client, const u64_snowflake_t guild_id,
                 ...
                 const struct discord_guild_member *member)
{
  ...
  discord_modify_guild_member(
    client, guild_id, member->user->id, &params3, NULL);

  return ret_role.id;
}

void
start_new_session(struct discord *client, const u64_snowflake_t guild_id,
                  const struct discord_guild_member *member)
{
  ...
  discord_create_reaction(
    client, session_channel_id, ret_msg.id, 0, g_session.reaction_emoji);

  discord_message_cleanup(&ret_msg);
}

void
send_next_question(struct discord *client, u64_snowflake_t channel_id,
                   struct session *session, struct question *question)
{
  char text[DISCORD_MAX_PAYLOAD_LEN];
  if (session->curr_question == g_session.questions_per_session) {
    sprintf(text,
            "You got %d out of %d! (%.1f%%)",
            session->hits,
            g_session.questions_per_session,
            100 *
  ...
  int offset = sprintf(
    text, "QUESTION %d\n%s\n", 1 + session->curr_question, question->desc);
...
void
on_reaction_add(struct discord *client, const struct discord_user *bot,
                ...
                const struct discord_emoji *emoji)
{
  ...
  if ((message_id == g_session.message_id) &&
      (0 == strcmp(emoji->name,
                   g_session.reaction_emoji))) { // close existing quiz session
...
main(int argc, char *argv[])
{
  ...
  discord_create_reaction(client,
                          g_session.channel_id,
                          g_session.message_id,
                          0,
                          g_session.reaction_emoji);
