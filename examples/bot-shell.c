...
void
on_cd(struct discord *client, const struct discord_user *bot,
...
void
on_less_like(struct discord *client, const struct discord_user *bot,
             const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
  ...
  if (*msg->content) {
    ...
    snprintf(
      attachment_url, sizeof(attachment_url), "attachment://%s", msg->content);
...
void
on_default(struct discord *client, const struct discord_user *bot,
           const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
