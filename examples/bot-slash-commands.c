    ...
    if (0 == strcasecmp(cmd_action, "LIST")) {
      ...
      if (guild_id)
        code = discord_get_guild_application_commands(
          client, g_application_id, guild_id, &app_cmds);
      else
      ...
      u64_snowflake_t guild_id = 0;
      sscanf(buf + bufoffset,
             "%32[^[][%100[^]]] %" SCNu64,
             cmd_name,
             cmd_desc,
             &guild_id);
      if (!*cmd_name || !*cmd_desc)
        goto _help;
      ...
      if (ORCA_OK == code && app_cmd.id) {
        log_info(
          "Created command:\t%s (" PRIu64 ")", app_cmd.name, app_cmd.id);
      ...
      if (ORCA_OK == code && app_cmd.id) {
        log_info(
          "Edited command:\t%s (%" PRIu64 ")", app_cmd.name, app_cmd.id);
        discord_application_command_cleanup(&app_cmd);
      }
      else {
        log_error("Couldn't create command '%s'", cmd_name);
