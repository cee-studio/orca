...
void
on_list_voice_regions(struct discord *client, const struct discord_user *bot,
...
void
on_voice_join(struct discord *client, const struct discord_user *bot,
              const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
  ...
  discord_get_channel_at_pos(client,
                             msg->guild_id,
...
void
on_voice_kick(struct discord *client, const struct discord_user *bot,
              const struct discord_message *msg)
{
  if (msg->author->bot)
    return;
  ...
  if (!user_id) {
    ...
    snprintf(
      text, sizeof(text), "<@!%" PRIu64 "> has been kicked from VC", user_id);
  }
...
{
  log_info("User <@!%" PRIu64 "> has joined <#%" PRIu64 ">!",
           vs->user_id,
           vs->channel_id);
}
