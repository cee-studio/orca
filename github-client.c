...
static void
_github_presets_init(struct github_presets *presets, char *username,
...
struct github *
github_init(const char username[], const char token[],
            const char repo_config[])
{
  ...
  _github_presets_init(
    &new_client->presets, strdup(username), strdup(token), repo_config);

  github_adapter_init(
    &new_client->adapter, &new_client->conf, &new_client->presets);
...
github_get_tree_sha(struct github *client, char *commit_sha, char **p_sha)
{
  ...
  return github_adapter_run(
    &client->adapter,
    &(struct ua_resp_handle){ .ok_cb = &load_sha, .ok_obj = p_sha },
    NULL,
    HTTP_GET,
...
ORCAcode
github_create_tree(struct github *client, char *base_tree_sha,
...
ORCAcode
github_create_a_commit(struct github *client, char *tree_sha,
...
ORCAcode
github_create_a_branch(struct github *client, char *head_commit_sha,
                       char *branch)
{
...
ORCAcode
github_create_a_pull_request(struct github *client, char *branch,
                             char *pull_msg)
{
...
ORCAcode
github_get_repository(struct github *client, char *owner, char *repo,
