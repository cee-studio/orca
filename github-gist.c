                            ...
                            struct github_gist* gist)
{
  ...
  snprintf(
          fmt, sizeof(fmt),
          "(public): \"%s\", (description): \"%s\", (files): { (%s): { (content): \"%s\" }}",
          params->public, params->description, params->title, params->contents);
...
ORCAcode github_get_gist(struct github* client, char* id,
                         struct github_gist* gist)
{
  log_info("===get-a-gist===");
