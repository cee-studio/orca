...
static enum slack_sm_types eval_sm_type(char type[])
{
  // EVENT API PAYLOADS
...
static void send_acknowledge(struct slack_sm* sm, const char envelope_id[])
{
  char payload[512];
  size_t ret =
...
static void on_hello(struct slack_sm* sm, const char* text, size_t len)
{
  sm->is_ready = true;
  ...
  long interval_s = 0;
  json_extract((char*)text, len, "(debug_info.approximate_connection_time):ld",
               &interval_s);
...
static void on_events(struct slack_sm* sm, struct sized_buffer* data,
                      char str_type[])
{
...
static void on_connect_cb(void* p_sm, struct websockets* ws,
                          struct ws_info* info, const char* ws_protocols)
{
  log_info("Connected, WS-Protocols: '%s'", ws_protocols);
}

static void on_close_cb(void* p_sm, struct websockets* ws, struct ws_info* info,
...
static void on_text_cb(void* p_sm, struct websockets* ws, struct ws_info* info,
                       const char* text, size_t len)
{
  struct slack_sm* sm = p_sm;

  char type[64] = "", envelope_id[64] = "";
  json_extract((char*)text, len, "(type):s,(envelope_id):s", type, envelope_id);
  ...
  log_trace(
          ...
          type, (*event_type) ? " -> " : "", event_type, len,
          info->loginfo.counter);

  on_events(sm, &data, event_type);
}

static enum slack_event_handling_mode noop_event_handler(struct slack* a,
...
static void refresh_connection(struct slack_sm* sm)
{
