...
{
  // EVENT API PAYLOADS
  if (STREQ("message", type))
    return SLACK_SOCKETMODE_TYPE_MESSAGE;
  // INTERACTION PAYLOADS
  if (STREQ("block_actions", type))
    return SLACK_SOCKETMODE_TYPE_BLOCK_ACTIONS;
  if (STREQ("message_actions", type))
    return SLACK_SOCKETMODE_TYPE_MESSAGE_ACTIONS;
  if (STREQ("view_closed", type))
    return SLACK_SOCKETMODE_TYPE_VIEW_CLOSED;
...
on_hello(struct slack_sm *sm, const char *text, size_t len)
{
  ...
  json_extract((char *)text,
               len,
               "(debug_info.approximate_connection_time):ld",
               &interval_s);
...
static void
on_connect_cb(void *p_sm, struct websockets *ws, struct ws_info *info,
...
static void
on_close_cb(void *p_sm, struct websockets *ws, struct ws_info *info,
...
static void
on_text_cb(void *p_sm, struct websockets *ws, struct ws_info *info,
           const char *text, size_t len)
{
  ...
  json_extract(
    (char *)text, len, "(type):s,(envelope_id):s", type, envelope_id);
...
noop_event_handler(struct slack *a, struct sized_buffer *b,
