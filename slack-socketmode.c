...
send_acknowledge(struct slack_sm *sm, const char envelope_id[])
{
  ...
  log_info(
    ANSICOLOR(
      "SEND", ANSI_FG_BRIGHT_GREEN) " ACKNOWLEDGE (%d bytes) [@@@_%zu_@@@]",
    ret,
  ...
  size_t len)
{
  ...
  log_warn(
    ANSICOLOR("CLOSE", ANSI_FG_RED) " (code: %4d, %zu bytes): '%.*s'",
    wscode,
    len,
    (int)len,
    reason);
...
noop_event_handler(
  struct slack *a, struct sized_buffer *b, enum slack_sm_types d)
{
...
slack_sm_run(struct slack *client)
{
  ...
  ASSERT_S(
    WS_DISCONNECTED == ws_get_status(sm->ws),
