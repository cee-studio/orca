...
void
slack_webapi_init(struct slack_webapi *webapi, struct logconf *conf,
                  struct sized_buffer *token)
{
  ...
  char auth[128];
  int ret = snprintf(
    auth, sizeof(auth), "Bearer %.*s", (int)token->size, token->start);
  ASSERT_S(ret < sizeof(auth), "Out of bounds write attempt");

  ua_reqheader_add(webapi->ua, "Authorization", auth);
  ua_reqheader_add(
    webapi->ua, "Content-type", "application/x-www-form-urlencoded");
}
...
ORCAcode
slack_webapi_run(struct slack_webapi *webapi, struct sized_buffer *resp_body,
