...
typedef void (*slack_idle_cb)(
  struct slack *client, const char payload[], const size_t len);
...
void slack_sm_set_on_block_actions(
  struct slack *client, slack_idle_cb callback);
void slack_sm_set_on_message_actions(
  struct slack *client, slack_idle_cb callback);
void slack_sm_set_on_view_closed(struct slack *client, slack_idle_cb callback);
void slack_sm_set_on_view_submission(
  struct slack *client, slack_idle_cb callback);

void slack_sm_run(struct slack *client);
void slack_sm_shutdown(struct slack *client);

ORCAcode slack_apps_connections_open(
  struct slack *client, struct sized_buffer *p_resp);
...
ORCAcode slack_chat_post_message(
  struct slack *client,
